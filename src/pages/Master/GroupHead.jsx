...
import { getGroupHeads, saveGroupHeads, saveGroupHead } from '../../utils/storageManager';
import { generateId } from '../../utils/helpers';
import DataTable from '../../components/DataTable';
import ModalAlert from '../../components/ModalAlert';
import ModalForm from '../../components/ModalForm';

export default function GroupHead({ searchQuery, triggerAdd }) {
  ...
  const headers = ['Actions', 'Timestamp', 'GH-NO', 'Group Head'];

  useEffect(() => {
    setData(getGroupHeads());
  }, []);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return (
        ...
        item.ghNo?.toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery]);
  ...
  const handleDelete = (id) => {
    showAlert('confirm', 'Delete Group Head?', 'This will remove the category from your item selection options.', () => {
      const updated = data.filter(i => i.id !== id);
      saveGroupHeads(updated);
      setData(updated);
      showAlert('success', 'Deleted!', 'The Group Head has been removed successfully.');
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (validEntries.length === 0) {
      showAlert('error', 'Error', 'Please provide at least one group head name.');
      return;
    }
    ...
    if (editingId) {
      ...
      saveGroupHeads(updated);
      setData(updated);
      showAlert('success', 'Updated!', 'Group Head name has been updated.');
    } else {
      const newItems = validEntries.map((entry, index) => ({
        id: generateId() + index,
        name: entry.name,
        timestamp: new Date().toISOString(),
        ghNo: `GH-${String(data.length + index + 1).padStart(3, '0')}`
      }));
      
      const updatedData = [...data, ...newItems];
      saveGroupHeads(updatedData);
      setData(updatedData);
      showAlert('success', 'Success!', `${newItems.length} group head(s) added successfully.`);
    }
    setShowModal(false);
  };
  ...
  const renderRow = (item) => (
    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-center text-sm">
      ...
      <td className="px-4 py-3 text-gray-900 font-bold whitespace-nowrap">{item.ghNo}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3 relative">
      <div className="flex justify-between items-center border-b border-gray-100 pb-2">
        <div><span className="text-[10px] font-medium text-indigo-600 uppercase tracking-widest">{item.ghNo}</span><h3 className="text-sm font-medium text-gray-700 mt-1">{item.name}</h3></div>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit Group Head' : 'Register Group Heads'}
              ...
              <div key={index} className="flex gap-2 items-center group">
                <div className="flex-1 space-y-1">
                  ...
                  <input 
                    ...
                    placeholder="e.g. Raw Materials, Spares, Consumables..." 
                  />
                </div>
