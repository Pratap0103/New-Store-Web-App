...
import { getDepartments, saveDepartments, saveDepartment } from '../../utils/storageManager';
import { generateId } from '../../utils/helpers';
import DataTable from '../../components/DataTable';
import ModalAlert from '../../components/ModalAlert';
import ModalForm from '../../components/ModalForm';

export default function Department({ searchQuery, triggerAdd }) {
  ...
  const headers = ['Actions', 'Timestamp', 'DP-NO', 'Department'];

  useEffect(() => {
    setData(getDepartments());
  }, []);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => {
      ...
      return (
        ...
        item.dpNo?.toLowerCase().includes(q)
      );
    });
  }, [data, searchQuery]);
  ...
  const handleDelete = (id) => {
    showAlert('confirm', 'Delete Department?', 'This action will remove the department from your organizational records.', () => {
      const updated = data.filter(i => i.id !== id);
      saveDepartments(updated);
      setData(updated);
      showAlert('success', 'Deleted!', 'Department has been removed successfully.');
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (validEntries.length === 0) {
      showAlert('error', 'Error', 'Please provide at least one department name.');
      return;
    }
    ...
    if (editingId) {
      ...
      saveDepartments(updated);
      setData(updated);
      showAlert('success', 'Updated!', 'Department name has been modified.');
    } else {
      const newItems = validEntries.map((entry, index) => ({
        id: generateId() + index,
        name: entry.name,
        timestamp: new Date().toISOString(),
        dpNo: `DP-${String(data.length + index + 1).padStart(3, '0')}`
      }));
      
      const updatedData = [...data, ...newItems];
      saveDepartments(updatedData);
      setData(updatedData);
      showAlert('success', 'Success!', `${newItems.length} department(s) added successfully.`);
    }
    setShowModal(false);
  };
  ...
  const renderRow = (item) => (
    <tr key={item.id} className="hover:bg-gray-50 transition-colors text-center text-sm">
      ...
      <td className="px-4 py-3 text-gray-900 font-bold whitespace-nowrap">{item.dpNo}</td>
  ...
  const renderCard = (item) => (
    <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm space-y-3 relative">
      <div className="flex justify-between items-center border-b border-gray-100 pb-2">
        <div><span className="text-[10px] font-medium text-indigo-600 uppercase tracking-widest">{item.dpNo}</span><h3 className="text-sm font-medium text-gray-700 mt-1">{item.name}</h3></div>
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <ModalForm
        ...
        title={editingId ? 'Edit Department' : 'Register Departments'}
            ...
            {entries.map((entry, index) => (
              <div key={index} className="flex gap-2 items-center group">
                <div className="flex-1 space-y-1">
                  {index === 0 && <label className="block text-[10px] font-medium text-gray-700 uppercase tracking-tight">Department Name *</label>}
                  <input 
                    required 
                    type="text" 
                    value={entry.name} 
