...
import { Search, Filter, Calendar, RotateCcw, AlertTriangle, CheckSquare } from 'lucide-react';
...
export default function Rectify() {
  ...
  const pendingData = useMemo(() => {
    return records.filter(item => {
      // Stage 2 pending: has planned2 but no actual2
      if (!item.planned2 || item.actual2) return false;

      // Date filtering
      if (filters.fromDate || filters.toDate) {
        const itemDate = item.planned2 ? item.planned2.substring(0, 10) : '';
  ...
  const historyData = useMemo(() => {
    return records.filter(item => {
      // Stage 2 history: has planned2 and actual2
      if (!item.planned2 || !item.actual2) return false;

      // Date filtering
      if (filters.fromDate || filters.toDate) {
        const itemDate = item.actual2 ? item.actual2.substring(0, 10) : '';
        if (filters.fromDate && itemDate < filters.fromDate) return false;
        if (filters.toDate && itemDate > filters.toDate) return false;
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.remarks.trim()) {
      toast.error('Please enter rectification remarks.');
      return;
    }
    ...
    try {
      ...
      const updates = {
        ...
        status2: formData.status,
        remarks2: formData.remarks
      };

      updateTallyEntry(processingItem.id, updates);
      toast.success(`Successfully updated rectification for Indent ${processingItem.indentNumber}`);
  ...
  const pendingHeaders = [
    ...
    'Planned Date', 'Product Name', 'Bill No.', 'Qty', 'Party Name', 'Bill Amt', 'Status 1', 'Remarks 1'
  ];

  const historyHeaders = [
    'Timestamp', 'Indent No.', 'Project Name', 'Material In Date', 'Product Name',
    'Bill No.', 'Qty', 'Party Name', 'Bill Amt', 'Status 1', 'Remarks 1', 'Status 2', 'Remarks 2'
  ];

  const renderPendingRow = (item, index) => {
    const formattedDate = item.timestamp ? new Date(item.timestamp).toLocaleDateString('en-GB') : '-';
    const plannedDateStr = item.planned2 ? new Date(item.planned2).toLocaleDateString('en-GB') : '-';
    ...
    return (
      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
        ...
        <td className="px-4 py-2.5 text-center text-xs font-medium text-gray-500 max-w-[200px] truncate" title={item.remarks1}>{item.remarks1 || '-'}</td>
      </tr>
    );
  };
  ...
  const renderHistoryRow = (item, index) => {
    ...
    return (
      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
        ...
        <td className="px-4 py-2.5 text-center text-xs font-medium text-gray-600 max-w-[150px] truncate" title={item.remarks1}>{item.remarks1 || '-'}</td>
        <td className="px-4 py-2.5 text-center whitespace-nowrap">
          <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${item.status2 === 'Done' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
            {item.status2 || 'Not Done'}
          </span>
        </td>
    ...
    return (
      <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <div>
            <span className="text-[9px] text-gray-400 font-semibold block uppercase">Planned Rectify</span>
            <span className="font-bold text-gray-700">{item.planned2 ? new Date(item.planned2).toLocaleDateString('en-GB') : '-'}</span>
          </div>
        </div>

        <div className="p-2 bg-red-50 rounded-lg border border-red-100 text-xs">
          <span className="text-[9px] text-red-500 font-bold uppercase tracking-wider block">Audit Rejection Reason</span>
          <p className="font-medium text-red-700 italic">{item.remarks1 || 'No audit remarks provided.'}</p>
        </div>
    ...
    return (
      <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3">
        <div className="flex justify-between items-start border-b border-gray-100 pb-2">
          ...
          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${item.status2 === 'Done' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            {item.status2 || 'Not Done'}
          </span>
        </div>
        ...
        <div className="pt-2 border-t border-gray-100 space-y-1">
          <div>
            <span className="text-[8px] text-gray-400 font-bold uppercase block">Audit Rejection Reason</span>
            <p className="text-[10px] font-medium text-red-600 italic">{item.remarks1}</p>
          </div>
          <div>
            <span className="text-[8px] text-gray-400 font-bold uppercase block">Rectification Actions</span>
            <p className="text-[10px] font-medium text-gray-600 italic">{item.remarks2 || 'No rectification remarks.'}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="p-0 sm:p-1 md:p-3 space-y-2 md:space-y-3 flex flex-col h-full min-h-0">
      ...
      <div className="flex flex-col lg:flex-row items-start lg:items-center gap-2 lg:gap-3 w-full pb-2 border-b border-gray-100 px-2 md:px-0">
        <TabSwitcher
          ...
          tabs={[
            { id: 'pending', label: 'Pending Rectification', count: pendingData.length },
            { id: 'history', label: 'Rectify History', count: historyData.length }
          ]}
        />
      ...
      {processingItem && (
        ...
        >
          <div className="space-y-4">
            ...
            {/* Previous Rejection View */}
            <div className="p-3 bg-red-50 rounded-xl border border-red-100 space-y-1">
              <span className="text-[9px] text-red-500 font-extrabold uppercase block tracking-wider">Audit Rejection Remarks</span>
              <p className="text-xs font-semibold text-red-700 italic">{processingItem.remarks1}</p>
            </div>

            {/* Status Select */}
            <div className="space-y-1.5">
              <label className="text-xs font-extrabold text-gray-500 uppercase tracking-wide block">Rectification Status *</label>
              <select
              ...
              >
                <option value="Done">Done (Mistake Rectified)</option>
                <option value="Not Done">Not Done (Supplier adjustment pending)</option>
              </select>
            </div>

            {/* Remarks Input */}
            <div className="space-y-1.5">
              <label className="text-xs font-extrabold text-gray-500 uppercase tracking-wide block">Rectification Remarks *</label>
              <textarea
                placeholder="Detail changes made (e.g. Uploaded revised invoice, added credit note copy, adjusted rate...)"
