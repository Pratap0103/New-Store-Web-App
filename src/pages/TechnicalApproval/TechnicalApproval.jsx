...
import TechnicalPending from './TechnicalPending';
...
export default function TechnicalApproval() {
  ...
  const [filters, setFilters] = useState({ searchQuery: '', fromDate: '', toDate: '', department: '', groupHead: '' });

  useEffect(() => { setIndents(getIndents()); }, []);
  ...
  const flattenedData = useMemo(() =>
    indents.flatMap(indent =>
      indent.items.map(item => ({ ...indent, ...item, indentId: indent.id }))
    ).reverse()
  , [indents]);

  const pendingItems = flattenedData.filter(i => i.vendorRateInfo && !i.technicalApproval);
  const historyItems = flattenedData.filter(i => i.technicalApproval);
  ...
  return (
    <div className="p-0 sm:p-1 md:p-3 space-y-2 md:space-y-3 flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 flex flex-col bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        {activeTab === 'pending'
          ? <TechnicalPending data={pendingItems} filters={filters} refresh={refreshData} />
          : <TechnicalHistory data={historyItems} filters={filters} />
        }
      </div>
    </div>
  );
