...
export default function BillnotreceviedHistory({ data, onViewImage }) {
  const headers = [
    'Lift Number', 'Indent No.', 'PO Number', 'Vendor Name', 'Project Name',
    ...
    'Status of bill', 'Bill IMage(Document)'
  ];
  ...
  const renderRow = (item, index) => {
    ...
    return (
      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
        ...
        <td className="px-3 py-2.5 text-center text-xs font-black text-indigo-600 whitespace-nowrap">{item.billNo}</td>
        ...
        <td className="px-3 py-2.5 text-center whitespace-nowrap">
          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${item.statusOfBill === 'Ok' ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200'}`}>
            {item.statusOfBill || 'Ok'}
          </span>
        </td>
        <td className="px-3 py-2.5 text-center whitespace-nowrap">
          <button
            onClick={() => onViewImage(item.billImage || 'https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?w=500', 'Bill Document')}
            className="flex items-center justify-center gap-1 px-2.5 py-1 bg-green-50 text-green-700 rounded-lg hover:bg-green-600 hover:text-white transition-all font-semibold text-[10px] mx-auto shadow-sm"
          >
            <Eye size={12} />
            <span>View Bill</span>
          </button>
    ...
    return (
      <div key={item.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 space-y-3">
        <div className="flex justify-between items-start border-b border-gray-100 pb-2">
          ...
          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${item.statusOfBill === 'Ok' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            {item.statusOfBill || 'Ok'}
          </span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <div>
            <span className="text-[9px] text-gray-400 font-semibold block uppercase">Bill Number</span>
            <span className="font-bold text-indigo-600 truncate block">{item.billNo}</span>
          </div>
          <div>
        ...
        <div className="flex justify-end pt-2 border-t border-gray-100">
          <button
            onClick={() => onViewImage(item.billImage || 'https://images.unsplash.com/photo-1554415707-6e8cfc93fe23?w=500', 'Bill Document')}
            className="flex items-center gap-1 px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-600 hover:text-white transition font-semibold text-[10px]"
          >
            <Eye size={12} />
